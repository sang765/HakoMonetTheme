(function() {
    ...
    function interceptFileInput() {
        ...
        inputs.forEach(input => {
            ...
            // Add click event to show method selection dialog
    ...
    /**
     * Create method selection dialog for upload
     */
    function createMethodSelectionDialog(input) {
        debugLog('Creating method selection dialog for input:', input.id);

        const modal = document.createElement('div');
        modal.className = 'hmt-method-dialog-overlay';
        modal.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
        ...
        `;
        modal.innerHTML = `
            <div class="hmt-method-dialog" style="
                background: white;
                border-radius: 10px;
                box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3);
                max-width: 400px;
                ...
                text-align: center;
            ">
                <h3 style="margin: 0 0 15px 0; font-size: 18px; font-weight: 600;">Chọn phương thức upload</h3>
                <p style="margin: 0 0 20px 0; font-size: 14px; color: #666;">Bạn muốn upload ảnh từ đâu?</p>
                <div style="display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
                    <button class="hmt-method-file" style="
                        padding: 12px 24px;
                        border: 1px solid #ddd;
                        background: white;
                        border-radius: 5px;
                        ...
                        gap: 8px;
                        transition: all 0.2s;
                    " onmouseover="this.style.background='#f8f9fa'; this.style.borderColor='#007bff';" onmouseout="this.style.background='white'; this.style.borderColor='#ddd';">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="fill: #007bff;">
                    ...
                    <button class="hmt-method-url" style="
                        padding: 12px 24px;
                        border: 1px solid #ddd;
                        background: white;
                        border-radius: 5px;
                        ...
                        gap: 8px;
                        transition: all 0.2s;
                    " onmouseover="this.style.background='#f8f9fa'; this.style.borderColor='#28a745';" onmouseout="this.style.background='white'; this.style.borderColor='#ddd';">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="fill: #28a745;">
                            <path d="M13.73 5.27a.75.75 0 0 1 1.06 0l5.25 5.25a.75.75 0 0 1 0 1.06l-5.25 5.25a.75.75 0 1 1-1.06-1.06L18.44 12l-4.71-4.71a.75.75 0 0 1 0-1.06zm-4.94 0a.75.75 0 0 0-1.06 0L2.5 10.5a.75.75 0 0 0 0 1.06l5.23 5.25a.75.75 0 1 0 1.06-1.06L4.08 12l4.71-4.71a.75.75 0 0 0 0-1.06z"/>
                        </svg>
                        Upload từ URL
                    </button>
                </div>
                <div style="margin-top: 20px; text-align: left; font-size: 12px; color: #888;">
                    ...
                    <ul style="margin: 5px 0 0 20px; padding: 0;">
                        <li>Upload từ URL hỗ trợ các định dạng ảnh phổ biến</li>
                        <li>Ảnh sẽ được xử lý và crop giống như upload từ file</li>
                        <li>URL phải là đường dẫn trực tiếp đến file ảnh</li>
                    </ul>
                </div>
                <style>
                    @media (max-width: 480px) {
                        ...
                        .hmt-method-dialog div[style*="display: flex"] {
                            flex-direction: column !important;
                            gap: 8px !important;
                        }
                        .hmt-method-dialog button {
                            width: 100% !important;
                        }
                    }
                </style>
        ...
        const fileBtn = modal.querySelector('.hmt-method-file');
        const urlBtn = modal.querySelector('.hmt-method-url');
     ...
     */
    function createUrlInputDialog(input) {
        debugLog('Creating URL input dialog for input:', input.id);

        const modal = document.createElement('div');
        modal.className = 'hmt-url-dialog-overlay';
        modal.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
        ...
        `;
        modal.innerHTML = `
            <div class="hmt-url-dialog" style="
                background: white;
                border-radius: 10px;
                box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3);
                max-width: 500px;
                ...
                padding: 20px;
            ">
                <h3 style="margin: 0 0 15px 0; font-size: 18px; font-weight: 600;">Upload ảnh từ URL</h3>
                <div style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px; font-size: 14px; color: #666;">Nhập URL ảnh:</label>
                    <input type="url" class="hmt-url-input" style="
                        width: 100%;
                        padding: 10px;
                        border: 1px solid #ddd;
                        border-radius: 5px;
                ...
                <div style="display: flex; gap: 10px; justify-content: flex-end;">
                    <button class="hmt-url-cancel" style="
                        padding: 8px 16px;
                        border: 1px solid #ddd;
                        background: white;
                        border-radius: 5px;
                        cursor: pointer;
                    ">Hủy</button>
                    <button class="hmt-url-submit" style="
                        padding: 8px 16px;
                        border: none;
                        background: #007bff;
                        color: white;
                ...
                <div class="hmt-url-loading" style="display: none; margin-top: 15px; text-align: center;">
                    ...
                    <span style="margin-left: 10px; font-size: 14px; color: #666;">Đang tải ảnh...</span>
                </div>
                <div class="hmt-url-error" style="display: none; margin-top: 10px; color: #dc3545; font-size: 14px;"></div>
                <style>
                    @keyframes spin {
        ...
        const urlInput = modal.querySelector('.hmt-url-input');
        const submitBtn = modal.querySelector('.hmt-url-submit');
        const cancelBtn = modal.querySelector('.hmt-url-cancel');
        const loadingDiv = modal.querySelector('.hmt-url-loading');
        const errorDiv = modal.querySelector('.hmt-url-error');
        ...
        submitBtn.addEventListener('click', async () => {
            ...
            if (!isValidImageUrl(url)) {
                showError('URL không hợp lệ hoặc không phải là ảnh');
                return;
            }
            ...
            try {
                ...
                // Process the file through existing flow
                processImageFile(file, input);
            } catch (error) {
                showLoading(false);
                showError(error.message || 'Không thể tải ảnh từ URL');
            }
        });
        ...
        function showLoading(show) {
            ...
            submitBtn.textContent = show ? 'Đang tải...' : 'Tải ảnh';
        }
    }
    ...
    function isValidImageUrl(url) {
        try {
            ...
            // If no extension, check if it's a known image service
            if (!hasImageExtension) {
                const imageServices = [
                    'imgur.com', 'cloudinary.com', 'images.weserv.nl',
                    'i.imgur.com', 'cdn.cloudflare.com', 'cdn.jsdelivr.net'
                ];
    ...
    async function fetchImageFromUrl(url) {
        ...
        try {
            ...
            const proxyUrl = window.HMTConfig ? window.HMTConfig.getPreferredProxy() : 'images.weserv.nl';
            
            let fetchUrl = url;
            
            if (useProxy) {
                // Use weserv.nl proxy for better CORS handling
                if (proxyUrl === 'images.weserv.nl') {
                    const urlParam = encodeURIComponent(url);
                    fetchUrl = `https://images.weserv.nl/?url=${urlParam}&w=2000&h=2000&fit=contain`;
                } else if (proxyUrl === 'allOrigins.nl') {
                    fetchUrl = `https://allorigins.nl/get?url=${encodeURIComponent(url)}`;
                }
            }
            ...
            if (!contentType || !contentType.startsWith('image/')) {
                throw new Error('URL không dẫn đến file ảnh hợp lệ');
            }

            const blob = await response.blob();
            
            // Validate image size
            const maxSize = 10 * 1024 * 1024; // 10MB limit for downloaded images
            if (blob.size > maxSize) {
                throw new Error(`Ảnh quá lớn (${(blob.size / 1024 / 1024).toFixed(2)}MB). Vui lòng chọn ảnh nhỏ hơn 10MB.`);
            }
            ...
            debugLog('Image fetched successfully:', file.name, file.size, 'bytes');
            return file;

        } catch (error) {
            debugLog('Error fetching image from URL:', error);
     ...
     */
    function generateFilenameFromUrl(url, contentType) {
        try {
            const urlObj = new URL(url);
    ...
    /**
     * Process image file through existing flow
     */
    function processImageFile(file, input) {
        debugLog('Processing image file from URL:', file.name, file.size, 'bytes');

        if (input.id === 'user_avatar_file') {
            // Avatar logic
            if (file.type === 'image/gif') {
                debugLog('GIF detected from URL, uploading directly');
                uploadAvatar(file);
            } else {
                // Check aspect ratio
                const reader = new FileReader();
                reader.onload = function(e) {
                    ...
                    img.onload = function() {
                        const aspectRatio = img.naturalWidth / img.naturalHeight;
                        debugLog('Image dimensions from URL:', img.naturalWidth, 'x', img.naturalHeight, 'aspect:', aspectRatio);
                        if (Math.abs(aspectRatio - 1) < 0.01) {
                            debugLog('Image is 1:1, uploading directly');
                            uploadAvatar(file);
                        } else {
